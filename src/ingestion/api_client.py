...
# ── Khoảng thời gian cố định: 09/09/2025 → 09/09/2026 ───────────────────────
FIXED_START_TS = 1757376000  # 2025-09-09 00:00:00 UTC
FIXED_END_TS   = 1788912000  # 2026-09-09 00:00:00 UTC
...
def fetch_air_quality(start_ts: int, end_ts: int) -> list[dict]:
    ...
    Tham số:
        start_ts : Unix timestamp bắt đầu  (vd: 1757376000)
        end_ts   : Unix timestamp kết thúc (vd: 1788912000)

    Trả về:
        Danh sách các bản ghi, mỗi bản ghi là dict gồm:
        {
            "dt"      : int   – Unix timestamp
            "datetime": str   – Dạng đọc được, vd "2025-09-09 00:00:00"
...
def save_to_csv(records: list[dict], path: str = RAW_CSV_PATH) -> None:
    """Ghi đè toàn bộ records ra file CSV."""
...
if __name__ == "__main__":
    ...
    print(f"✅ Lấy được {len(data)} bản ghi. Ví dụ bản ghi đầu tiên:")
